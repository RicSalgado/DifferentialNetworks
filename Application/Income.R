...
data_cleaned <- subset(data_cleaned, select=-c(income_per_capita))

max_income <- max(data$income_per_capita)
min_income <- min(data$income_per_capita)

diff_income <- max_income - min_income

richer_states <- data[data$income_per_capita > (diff_income/2 + min_income),]
poorer_states <- data[data$income_per_capita < (diff_income/2 + min_income),]

richer_npn <- npn(richer_states)
poorer_npn <- npn(poorer_states)

results <- estimation(richer_npn, poorer_npn, nlambda = 50, tuning = "EBIC")
